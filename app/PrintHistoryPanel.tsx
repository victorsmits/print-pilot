...
export default function PrintHistoryPanel({ prints, loading, onClose }: { prints: PrintRun[]; loading: boolean; onClose: () => void }) {
  ...
  return <div className="drawer-backdrop" onMouseDown={onClose}><aside className="history-drawer" onMouseDown={event => event.stopPropagation()} role="dialog" aria-modal="true">
    ...
    {loading ? <p>Chargement…</p> : prints.length === 0 ? <div className="history-empty"><b>Aucune impression enregistrée</b><p>Après un tranchage ou une impression, reporte les vraies valeurs et le résultat dans l’étape Configuration.</p></div> : <div className="history-list">{prints.map(run => <article key={run.id} className={`history-run ${run.outcome}`}><div><span>{new Date(run.createdAt).toLocaleDateString("fr-FR")}</span><b>{run.name}</b><small>{run.sourceFileName ?? "Projet sans fichier"}</small></div><div className="history-metrics"><span>{run.durationMinutes != null ? `${Math.floor(run.durationMinutes / 60)} h ${run.durationMinutes % 60} min` : "Temps —"}</span><span>{run.filamentUsedG != null ? `${run.filamentUsedG.toFixed(1)} g` : "Masse —"}</span><span>{run.totalCost != null ? `${run.totalCost.toFixed(2)} €` : "Coût —"}</span></div><footer><b>{OUTCOME[run.outcome]}</b><span>{run.qualityRating ? `${"★".repeat(run.qualityRating)}${"☆".repeat(5 - run.qualityRating)}` : "Non notée"}</span></footer>{(run.defects || run.notes) && <p>{[run.defects, run.notes].filter(Boolean).join(" · ")}</p>}</article>)}</div>}
  </aside></div>;
}
