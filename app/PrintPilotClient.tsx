...
export default function PrintPilotClient({ user }: { user: AccountUser }) {
  ...
  function exportProject() {
    ...
    setExportStatus(`3MF v4 créé avec le profil Creality Hi complet : ${project.appliedKeys.length} réglages seulement ont été modifiés par PrintPilot. Ouvre-le comme projet.`);
  }
  ...
  return <main><TutorialPanel item={tutorial} onClose={() => setTutorial(null)} />
    ...
    <section className="workspace"><div className="stage">
      ...
      {step === 4 && <div className="step-panel result-panel">
        ...
        <section className="export-card"><div><span>PROJET 3MF V4</span><h3>Exporter le STL avec un profil complet</h3><p>Le modèle conserve son orientation. Le 3MF embarque la configuration Creality Hi officielle complète, puis remplace seulement les décisions affichées ci-dessus.</p></div><button className="primary" onClick={exportProject} disabled={!mesh || nozzle !== "0.4"}>Exporter le projet 3MF</button>{exportStatus && <p className="export-status">{exportStatus}</p>}<small>Export ciblé : Creality Hi, buse 0,4 mm, mono-matériau. N’utilise plus les fichiers PrintPilot v3. Pour une impression multicolore, assigne les filaments dans Creality Print et laisse le logiciel gérer la tour de purge. Vérifie ensuite l’aperçu couche par couche.</small></section>
