...
export default function InventoryPanel({ inventory, loading, onReload, onClose }: Props) {
  ...
  const [query, setQuery] = useState(""), [materialFilter, setMaterialFilter] = useState("all"), [brandFilter, setBrandFilter] = useState("all"), [statusFilter, setStatusFilter] = useState("all"), [sortOrder, setSortOrder] = useState("recent");
  ...
  const materials = useMemo(() => [...new Set(inventory.map(row => row.family).filter(Boolean))].sort((a, b) => a.localeCompare(b, "fr")), [inventory]);
  const brands = useMemo(() => [...new Set(inventory.map(row => row.brand).filter((value): value is string => Boolean(value)))].sort((a, b) => a.localeCompare(b, "fr")), [inventory]);
  const visibleInventory = useMemo(() => {
    const needle = query.trim().toLocaleLowerCase("fr");
    return inventory.filter(row => {
      const searchable = `${row.brand ?? ""} ${row.productLine ?? ""} ${row.colorName ?? ""} ${row.family} ${row.storageLocation ?? ""}`.toLocaleLowerCase("fr");
      if (needle && !searchable.includes(needle)) return false;
      if (materialFilter !== "all" && row.family !== materialFilter) return false;
      if (brandFilter !== "all" && row.brand !== brandFilter) return false;
      if (statusFilter === "calibrated" && !row.calibrated) return false;
      if (statusFilter === "uncalibrated" && row.calibrated) return false;
      if (statusFilter === "low" && (row.remainingG == null || row.remainingG >= 250)) return false;
      return true;
    }).sort((a, b) => {
      if (sortOrder === "name") return a.label.localeCompare(b.label, "fr");
      if (sortOrder === "color") return (a.colorName ?? "").localeCompare(b.colorName ?? "", "fr");
      if (sortOrder === "remaining") return (b.remainingG ?? -1) - (a.remainingG ?? -1);
      return (b.dbId ?? 0) - (a.dbId ?? 0);
    });
  }, [inventory, query, materialFilter, brandFilter, statusFilter, sortOrder]);
  ...
  return <div className="drawer-backdrop" onMouseDown={onClose}><aside className="inventory-drawer" onMouseDown={event => event.stopPropagation()} role="dialog" aria-modal="true">
    ...
    <div className="inventory-layout"><div className="inventory-sidebar"><button className="new-spool" onClick={() => choose(undefined)}>＋ Ajouter une bobine</button><section className="inventory-filters" aria-label="Filtres de l’inventaire"><label className="inventory-search"><span>Rechercher</span><input type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Couleur, marque, matière…"/></label><div><label><span>Matière</span><select value={materialFilter} onChange={event => setMaterialFilter(event.target.value)}><option value="all">Toutes</option>{materials.map(material => <option key={material}>{material}</option>)}</select></label><label><span>Marque</span><select value={brandFilter} onChange={event => setBrandFilter(event.target.value)}><option value="all">Toutes</option>{brands.map(brand => <option key={brand}>{brand}</option>)}</select></label><label><span>État</span><select value={statusFilter} onChange={event => setStatusFilter(event.target.value)}><option value="all">Tous</option><option value="calibrated">Calibrées</option><option value="uncalibrated">À calibrer</option><option value="low">Moins de 250 g</option></select></label><label><span>Trier</span><select value={sortOrder} onChange={event => setSortOrder(event.target.value)}><option value="recent">Plus récentes</option><option value="name">Nom</option><option value="color">Couleur</option><option value="remaining">Quantité</option></select></label></div><div className="inventory-filter-result"><b>{visibleInventory.length} / {inventory.length}</b><button type="button" onClick={resetFilters}>Réinitialiser</button></div></section>{loading && <p>Chargement…</p>}{!loading && inventory.length > 0 && visibleInventory.length === 0 && <p className="inventory-no-result">Aucune bobine ne correspond aux filtres.</p>}{visibleInventory.map(row => <button key={row.id} className={selectedId === row.dbId ? "active" : ""} onClick={() => choose(row)}><i className="inventory-swatch" style={{ background: row.colorHex ?? "#bac0ba" }} title={row.colorHex ?? "Couleur non renseignée"}></i><span><b>{row.brand} {row.productLine}</b><small>{row.colorName} · {row.family}</small></span><em>{row.remainingG == null ? "? g" : `${Math.round(row.remainingG)} g`}</em></button>)}</div>
      <form className="spool-form" onSubmit={submit}><div className="form-heading"><div className="inventory-title"><i className="inventory-color-preview" style={{ background: String(form.colorHex || "#bac0ba") }}></i><div><span className="eyebrow">{selectedId ? "MODIFIER" : "NOUVELLE BOBINE"}</span><h3>{selected?.label ?? "Créer une fiche complète"}</h3><small>{String(form.colorName || "Couleur non renseignée")} · {String(form.colorHex || "—")}</small></div></div>{selectedId && <button type="button" className="danger-link" onClick={remove}>Supprimer</button>}</div>
