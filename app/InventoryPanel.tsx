  ...
  async function importInvoice(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setForm(current => ({ ...current,
        supplier: draft.supplier || current.supplier, invoiceNumber: draft.invoiceNumber || current.invoiceNumber, purchaseDate: draft.purchaseDate || current.purchaseDate,
        purchaseTotal: draft.purchaseTotal || current.purchaseTotal, brand: draft.brand || current.brand, productLine: draft.productLine || current.productLine,
        material: draft.material || current.material, colorName: draft.colorName || current.colorName, spoolWeightG: draft.spoolWeightG || current.spoolWeightG,
        remainingG: current.remainingG || draft.spoolWeightG, pricePerKg: draft.pricePerKg || current.pricePerKg,
      }));
      setInvoiceWarnings(draft.warnings);
      setMessage("Facture analysée localement. Vérifie les champs surlignés avant d’enregistrer.");
